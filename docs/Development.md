...
`ActivityArtwork.swift` maps each saved movement to `BrackenActivities` atlas frames or individual `Bracken…` imagesets. `ActivityDefinition.catalog` registers the built-in activities available through the searchable library; adding one uses the normal configuration save. Keep IDs stable and supply a distinct pose, target and instructions for each movement. Poses are independent of editable wording, and custom activities can choose any illustration. Check library, card and session crops at phone/Watch scale, including accessibility text sizes.
